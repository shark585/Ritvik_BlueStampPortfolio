...
For my second milestone, I was able to display the simulation on the display provided just by running the Python script. Luckily, I didn't encounter many errors and most of my errors were simple fixes that weren't too tedious, which was quite surprising to me. The only main challenge I faced was getting the video to display during runtime, but I was able to use the subprocess package in Python to call command lines in the terminal. The biggest obstacle I faced was showing the graph for the learning curve, since it would just show an empty graph. This ended up being a simple fix, however, since I had set the number of episodes (times it would attempt to land) to a small number, which meant that there weren't enough data points to use. I just had to increase the count to get a proper graph to show. My next goal is to make it more user-interactive, where the user can create their own terrain for the lander to land on. 
